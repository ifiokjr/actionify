 ...
 * `pre-if` defaults to `always()`. Note that the `step` context is unavailable,
 * as no steps have run yet.
 */
...
export interface GitHubAction {
  ...
  inputs?: {
     ...
     * unique identifier within the inputs object. The `<input_id>` must start
     * with a letter or `_` and contain only alphanumeric characters, `-`, or
     * `_`.
     *
     * This interface was referenced by `undefined`'s JSON-Schema definition via
     * the `patternProperty` "^[_a-zA-Z][a-zA-Z0-9_-]*$".
     */
    [k: string]: {
      /**
      ...
      required?: boolean;
      /**
       * A string representing the default value. The default value is used when
       * an input parameter isn't specified in a workflow file.
       */
      default?: string;
    };
  };
...
/**
 * Configures the path to the action's code and the application used to execute
 * the code.
 */
export interface RunsJavascript {
  ...
  using: "node12" | "node16";
  /**
   * The file that contains your action code. The application specified in
   * `using` executes this file.
   */
  main: string;
  /**
   * Allows you to run a script at the start of a job, before the `main:` action
   * begins. For example, you can use `pre:` to run a prerequisite setup script.
   * The application specified with the `using` syntax will execute this file.
   * The `pre:` action always runs by default but you can override this using
   * `pre-if`.
   */
  pre?: string;
  "pre-if"?: PreIf;
   ...
   * completed. For example, you can use `post:` to terminate certain processes
   * or remove unneeded files. The application specified with the `using` syntax
   * will execute this file. The `post:` action always runs by default but you
...
/**
 * Configures the path to the composite action, and the application used to
 * execute the code.
 */
...
export interface RunsDocker {
   ...
   * be the Docker base image name, a local `Dockerfile` in your repository, or
   * a public image in Docker Hub or another registry. To reference a
   * `Dockerfile` local to your repository, use a path relative to your action
   * metadata file. The `docker` application will execute this file.
   */
  image: string;
  /**
   ...
   * wasn't already specified. Use `entrypoint` when the `Dockerfile` does not
   * specify an `ENTRYPOINT` or you want to override the `ENTRYPOINT`
   * instruction. If you omit `entrypoint`, the commands you specify in the
   * Docker `ENTRYPOINT` instruction will execute. The Docker `ENTRYPOINT
  ...
  entrypoint?: string;
  /**
   * Allows you to run a script before the `entrypoint` action begins. For
   * example, you can use `pre-entrypoint:` to run a prerequisite setup script.
   * GitHub Actions uses `docker run` to launch this action, and runs the script
   * inside a new container that uses the same base image. This means that the
   * runtime state is different from the main `entrypoint` container, and any
   * states you require must be accessed in either the workspace, `HOME`, or as
   * a `STATE_` variable. The `pre-entrypoint:` action always runs by default
   ...
   * image, the runtime state is different from the main `entrypoint` container.
   * You can access any state you need in either the workspace, `HOME`, or as a
   * `STATE_` variable. The `post-entrypoint:` action always runs by default but
   ...
   * can include hardcoded strings. GitHub passes the `args` to the container's
   * `ENTRYPOINT` when the container starts up. The `args` are used in place of
   * the `CMD` instruction in a `Dockerfile`. If you use `CMD` in your
   * `Dockerfile`, use the guidelines ordered by preference:
   ...
   * - If the action exposes a `--help` flag, or something similar, use that to
   *   make your action self-documenting.
   */
  args?: string[];
}
